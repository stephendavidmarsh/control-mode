...
(defun control-mode-get-converted-keymap-for (keymap)
  (let ((auto-keymap (make-sparse-keymap)))
    ...
    (cl-flet* ((add-binding (e b) (define-key auto-keymap (vector e) b))
               (key-bindingv (e) (key-binding (vector e)))
               (mod-modifiers (event f)
                              (event-convert-list (append (funcall f (remq 'click (event-modifiers event))) (list (event-basic-type event)))))
               (remove-modifier (event mod) (mod-modifiers event (lambda (x) (remq mod x))))
               (add-modifier (event mod) (mod-modifiers event (lambda (x) (cons mod x))))
               (add-modifiers (event mod1 mod2) (mod-modifiers event (lambda (x) (cons mod2 (cons mod1 x)))))
